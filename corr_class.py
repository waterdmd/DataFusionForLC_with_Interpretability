...
def plot_bar_charts(pos_counts, neg_counts, class_labels, title, filename):
    ...
    ax1.bar(x, pos_values, color='blue', label='Positive Impact')
    ax1.bar(x, neg_values, color='red', label='Negative Impact')
    ...
    ax2.bar(x, difference, color='purple', label='Difference (Positive - Negative)')
...
for target_class in total_positive_counts.keys():
    ...
    plot_bar_charts(pos_counts, neg_counts, class_labels,
                    f'Bar Plot for Class {target_class}', os.path.join(output_dir, f'bar_plot_class_{target_class}.png'))
